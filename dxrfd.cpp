...
// R3ABM patch begin
         ...
         if ((a_user_ptr->version & 2) == 0)
         {
           hb_buf[7] = ' ';
           hb_buf[8] = '\0';

           sendto(srv_sock, hb_buf, 9, 0,
             (struct sockaddr *)&(a_user_ptr->sin),
             sizeof(struct sockaddr_in));
         }
// R3ABM patch end
...
/***************************************************************************************************/
      ...
      if (FD_ISSET(srv_sock, &fdset))
      {
         ...
         {
            // DL3OCK patch begin
            if ( (recvlen == CALL_SIZE + 3) && (readBuffer[(recvlen - 1)] == 11) )
            {
              UP4DAR_candidat = true;
            }
            else
            {
              UP4DAR_candidat = false;
            }
            // DL3OCK patch end
                     ...
                     if (readBuffer[9] == ' ')
                     {
                        ...
                        if (i == 5)
                        {
                           ...
                           */
                           ...
                           // DL3OCK patch begin
                           sendto(srv_sock,"DISCONNECTED", 12,
            ...
            else
            {
               ...
               if (allowed_to_connect)
               {
                  ...
                  if (a_user_ptr)
                  {
                     ...
                     /* 
                        OE2AIP DL3OCK
                        UP4DAR Client expect response to his source port and not to the default 30001 
                        This solves some problems with NAT.
                     */
                     if (UP4DAR_candidat == true)
                        a_user_ptr->up4dar = true;
                     else 
                     {
                        a_user_ptr->up4dar = false;
                     ...
                     {
                        // OE2AIP patch begin
                        if (a_user_ptr->up4dar)
                        {
                           traceit("new CALL=%s,UP4DAR based REPEATER,ip=%s,users=%d, link from mod %c ---> ref mod %c\n",
                             a_call,
                             an_ip,
